...
function renderRecent(){
  ...
  const weighted = projects.map(p=>{
    ...
    let w = 1e15;
    if(n.date){
      const now = new Date(); const diff = n.date - now;
...
(function initQuote(){
  ...
  elMultiToggle?.addEventListener('change', ()=>{
    document.getElementById('qa-multi').classList.toggle('hidden', !elMultiToggle.checked);
  ...
  if(kind==='changes'){
    ...
    editorForm.innerHTML = `
      <div class="card" style="margin-bottom:10px">
        <div class="section-head"><h3>本次修改</h3></div>
        <div class="h-row">
          <label>关联阶段
...
function rowHTML(p){
  ...
  const lastText = last ? `[${last.phase}·${last.version}] ${last.text.slice(0,80)}${last.text.length>80?'…':''}` : '—';
  return `
    <tr>
      <!-- 项目 -->
...
function renderProjects(){
  const tbA = document.getElementById('projects-body-active');
  const tbD = document.getElementById('projects-body-done');
  tbA.innerHTML=''; tbD.innerHTML='';

  const active = projects.filter(p=> (p.pay_status||'未收款') !== '已收尾款');
  const done   = projects.filter(p=> (p.pay_status||'未收款') === '已收尾款');

  active.forEach(p=> tbA.insertAdjacentHTML('beforeend', rowHTML(p)));
  done.forEach(p=> tbD.insertAdjacentHTML('beforeend', rowHTML(p)));

  bindProjectTable(tbA);
  bindProjectTable(tbD);

  // —— 渲染后执行“溢出缩字”
  [tbA, tbD].forEach(tb=> shrinkOverflowCells(tb));
}
...
function openQuickModal(id){
  ...
  quickBody.innerHTML = `
    <div class="grid-1">
      ...
      <div class="small muted">规格：${(typeSpecLines(p)||['—']).join(' / ')}</div>
...
// ============== 作品合集：超大海报 + 自动抽帧封面（尽力，CORS 不通过则回退占位） ==============
...
async function getCoverFor(p){
  // 未来可扩展：若你在表里新增 poster_url 字段，这里优先用 poster_url
  if(thumbCache.has(p.final_link)) return thumbCache.get(p.final_link);
  let cover = null;
  if(p.final_link && /\.(png|jpe?g|webp)$/i.test(p.final_link)){
    cover = p.final_link;
  }else if(p.final_link && /\.(mp4|mov|m4v|webm|ogg)$/i.test(p.final_link)){
    cover = await captureVideoFrame(p.final_link);
  }
  if(!cover){
    // 占位渐变
    cover = 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(
      `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1600 900"><defs><linearGradient id="g" x1="0" x2="1"><stop stop-color="#f2f2f7"/><stop offset="1" stop-color="#e9ecef"/></linearGradient></defs><rect fill="url(#g)" width="1600" height="900"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-family="Arial" font-size="64" fill="#999">No Cover</text></svg>`
    );
  }
  thumbCache.set(p.final_link, cover);
  return cover;
}
async function renderGallery(){
  ...
  if(finals.length===0){
    const ph = document.createElement('div'); ph.className='mag-card';
    ph.innerHTML = `<div class="mag-cap">暂未上传成片，请在项目中填写 Final 链接</div>`;
    grid.appendChild(ph);
    return;
  }
  for(const p of finals){
    const a = document.createElement('a');
    a.className='mag-card'; a.href=p.final_link; a.target='_blank';
    const cover = await getCoverFor(p);
    a.innerHTML = `
      <img class="mag-cover" src="${cover}" alt="">
      <div class="mag-cap">${p.title||'未命名'}${p.brand?` · ${p.brand}`:''}</div>
    `;
    grid.appendChild(a);
  }
}
...
function renderCalendar(){
  ...
  projects.forEach(p=>{
    ...
    [['a_copy','a','Acopy',vers.A],['b_copy','b','Bcopy',vers.B],['final_date','final','Final',vers.F]].forEach(([key,typ,label,ver])=>{
      ...
      (evs[day] ||= []).push({ typ, txt:`${p.title||'未命名'} · ${label}${ver?(' '+ver):''}`, overdue });
    });
  });
