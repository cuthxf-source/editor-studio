...
async function fetchProjects(){
  const { data, error } = await supa
    .from('projects')
    // v1.4.6 兼容：不选择具体列，避免缺列导致整表查询失败（如 poster_url、deposit_amount）
    .select('*')
    .order('updated_at',{ ascending:false })
    .limit(1000);
  if(error){ console.error('[fetchProjects]', error); projects = []; return; }
  projects = data || [];
}
...
function renderFinance(){
  ...
  projects.forEach(p=>{
    ...
    const sum = Number(p.paid_amount||0); // 统计按已收款
...
document.getElementById('new-form')?.addEventListener('submit', async (e)=>{
  ...
  // v1.4.6：不再强写不存在的列（如 deposit_amount），以兼容你的现有表结构
