/* editor-studio / app.js  v1.4.3.1 */

// ============== App 版本（用于修改记录） ==============
const APP_VERSION = 'v1.4.3.1';
...
async function fetchProjects(){
  // 先按 updated_at 排序，若失败（列不存在），回退到 id 排序
  ...
  ({ data, error } = await supa
    ...
    .limit(1000));
  if(error){
    console.warn('Fallback order by id due to:', error.message);
    ({ data, error } = await supa
      .from('projects')
      .select('*')
...
(function initQuote(){
  ...
  document.getElementById('qa-mc-add')?.addEventListener('click', ()=>{
...
function comboRowHTML(idx,c){
  ...
  <div class="combo-row" data-idx="${idx}">
    ...
    <div class="center pill-group" style="flex:1 1 100%;">${checkboxList(RATIO_OPTS, c.ratios||[])}</div>
...
function renderFinance(){
  ...
  const container = document.getElementById('trend'); if(container) drawTrend(container, series);
}
...
document.getElementById('new-form')?.addEventListener('submit', async (e)=>{
  ...
  row.deposit_amount = 0; // 兼容旧结构，不再使用
