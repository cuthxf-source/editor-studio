...
/* 首页：最近项目（按优先级排序；去掉右侧图标；最右显示版本号胶囊） */
...
function renderKpis(){
  ...
  $('home-asof').textContent=s; $('f-asof').textContent=s;
}

/* 报价分析器（延续 v1.4.8，默认多组合） */
(function initQuote(){
  ...
  const list=$('qa-mc-list'), addBtn=$('qa-mc-add');
  function unitPriceBy(type, secs, o={}){ const d=typeBase[type]||{price:0,baseSec:0,secRate:0}; const base=Number(o.base||d.price); const over=Math.max(0,Number(secs||0)-d.baseSec); let price=base*(over>0?Math.pow(1+d.secRate,over):1); if(o.compOn){ price*=1+Number(o.comp||0)/100; } price*=1+Number(o.creative||0)/100; price*=1+(Number(o.urgent||0)/10)*0.03; const extra=Math.max(0,Number(o.rev||4)-4); price*=1+extra*0.2; return Math.round(price); }
  function mcRowHTML(idx,c){ return `
    <div class="card" data-idx="${idx}" style="margin:10px 0">
      <div class="h-row">
        ...
        <label>基础价（可填）<input class="mc-base" type="number" min="0" step="1" value="${c.base||''}" placeholder="默认"></label>
      </div>
      <div class="h-row mc-adv">
        <label class="pill"><input type="checkbox" class="mc-comp-ck" ${c.compOn?'checked':''}><span>启用合成</span></label>
        <label>合成复杂度（%）<input class="mc-comp" type="range" min="0" max="100" step="10" value="${c.comp||0}"></label>
        <label>创意密度（%）<input class="mc-creative" type="range" min="0" max="100" step="1" value="${c.creative||30}"></label>
        <label>紧急系数（%）<input class="mc-urgent" type="range" min="0" max="100" step="10" value="${c.urgent||0}"></label>
        <label>修改次数<input class="mc-rev" type="number" min="0" step="1" value="${c.rev||4}"></label>
      </div>
    </div>`;}
  ...
  function rowOpts(row){ return {base:row.querySelector('.mc-base').value, compOn:row.querySelector('.mc-comp-ck').checked, comp:row.querySelector('.mc-comp').value, creative:row.querySelector('.mc-creative').value, urgent:row.querySelector('.mc-urgent').value, rev:row.querySelector('.mc-rev').value}; }
  function calc(){ const rows=getRows(); const net=rows.reduce((s,r)=> s + Number(r.querySelector('.mc-count').value||1)*unitPriceBy(r.querySelector('.mc-type').value, Number(r.querySelector('.mc-secs').value||0), rowOpts(r)),0); $('qa-net').textContent=money(net); $('qa-gross').textContent=money(Math.round(net*1.06)); }
  addBtn.addEventListener('click',()=>{ const idx=list.querySelectorAll('[data-idx]').length; list.insertAdjacentHTML('beforeend', mcRowHTML(idx,{type:'LookBook',count:1,secs:0,creative:30,urgent:0,rev:4,compOn:false,comp:0})); calc(); });
  $('quote-form').addEventListener('input', calc);
  ensureOneRow(); calc();
})();

/* 编辑模态（只改“修改内容”为粘贴框 + 历史可编辑） */
  ...
  if(kind==='changes'){
    ...
    if(editTs){ // 覆盖已有记录
      ...
      if(idx>=0){
        ...
        d.changes[idx].imgs.push(...urls); // 新图追加
...
function renderRow(p){
  ...
  tr.innerHTML = `
    ...
    <td><div class="cell-summary"><button class="pill ${payClass} pay-pill" data-id="${p.id}" data-st="${pay}">${pay}</button></div></td>
...
/* 快速查看 / 作品合集 / 日历 / 财务（承接 v1.4.8，无改动要点） */
  ...
  projects.forEach(p=>{
    ...
    spans.forEach(sp=>{
      ...
      for(let d=s.getDate(); d<=e.getDate(); d++){
        ...
        const lane=(p.id%3); const line=document.createElement('div'); line.className='bar-line'; line.style.marginTop=(6+lane*10)+'px';
        line.appendChild(piece); dayCells[d]?.appendChild(line);
      }
    });
  });
}
